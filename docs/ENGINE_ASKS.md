...
## 34. Peer route-reachability probing — `Device::route_check{,_probe}()` + a probe hint on `suggest_exit_node()` (for `tailscale routecheck` and `exit-node suggest --force-probe`)
