...
| `debug` rich reads (`netmap`/`derp-map`/`control-knobs`) + magicsock knobs (`rotate-disco-key`, `derp-set-on-demand`, `break-*-conns`, `force-netmap-update`, `peer-endpoint-changes`, `set-expire`, `ts2021`, `dial-types`, `peer-relay-servers`) | `tsd-b15` | — | Each needs a netmap field or a magicsock knob the engine doesn't expose. (`portmap` is now SHIPPED — `tnet debug portmap`, on the daemon's own port-mapping client in `src/portmap`; it needs nothing from the engine because the conversation is with the LAN router, not the tailnet.) Re-confirmed against pin `9d847a6e`/v0.43.0. The pure-local cherry-picks (`prefs`/`env`/`via`/`local-creds`/`stat`/`restun`/`statedir`/`build-info`) are all shipped — `tsd-b15` has no daemon-buildable work left. |
...
| **Port mapper** (UPnP-IGD / NAT-PMP / PCP) | `tsd-vxb` | **Client shipped** (`src/portmap`, ported from Go `net/portmapper`): NAT-PMP (RFC 6886), PCP (RFC 6887) and UPnP-IGD (SSDP discovery + SOAP `AddAnyPortMapping`/`AddPortMapping`/`GetExternalIPAddress`), plus gateway discovery and the `debug portmap` diagnostic that drives them (`tnet debug portmap`, Go `feature/debugportmapper`). What is NOT closed, and cannot be daemon-side: **feeding the mapped endpoint to magicsock**, which is engine-owned and has no port-mapping code or endpoint-injection seam at pin `9d847a6e`. So a mapping can be obtained and reported, but not yet advertised to peers. |
