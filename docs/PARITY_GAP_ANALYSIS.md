...
support); **engine-gated features** (the four residual Linux-router `up`/`set` pref flags — the other
eight shipped — tailnet-lock key-set mutation, the incremental peer-delta bus, the mutating web UI,
and a Taildrop file-arrival signal);
...
flowchart TB
    ...
    subgraph OPEN["Open (12)"]
        O["#5 macOS utun default name (daemon works around) ·<br/>#8 exit-node DNS advertise side · #13 Funnel type re-export ·<br/>#18 Windows host route/DNS · #20 Taildrop file-arrival signal ·<br/>#21 4 residual Linux-router pref-flag Config fields · #25 TKA add/remove/log ·<br/>#27 tka_local_disable · #28 incremental peer deltas ·<br/>#29 web-client session auth · #30 serve_path segment-boundary bug ·<br/>#31 Taildrop send-path (chunked body, progress, target reason)"]
    end
