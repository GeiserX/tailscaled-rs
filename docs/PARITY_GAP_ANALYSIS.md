...
| `file cp` residual Go-fidelity gaps (stdin streaming, rich pre-send errors, offline-warning, system-DNS fallback) | `tsd-52k` | Shipped: `file cp -` reads stdin, the destination is resolved Go's way (netmap then system DNS), and a new read-only `file_cp_target` LocalAPI verb carries Go's `getTargetStableID`/`fileTargetErrorDetail` outcome — so an offline peer gets Go's `# warning: <target> is offline` before the send and an ineligible one gets Go's verbatim reason instead of "no tailnet peer matches". Residual: stdin is **spooled** to a temp file rather than streamed, because the engine's `send_file` declares a fixed `Content-Length` and has no chunked/unknown-length mode (engine ask #31); Go sends `-1` and streams. |
