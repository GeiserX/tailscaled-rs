...
pub struct ConfigVAlpha {
    ...
    /// ([`crate::routes::validate_advertise_routes`]): a 4via6 prefix must decode, and a default
    /// route must appear in both families or in neither (a lone `0.0.0.0/0` is a half exit node).
    ///
    ...
    /// empty (`AdvertiseRoutesSet = true`) and withdraws every advertised **subnet route**. Only an
    /// absent key (or JSON `null`) leaves the persisted routes alone. A bare `Vec` cannot tell the
    /// two apart, and would make "stop advertising these subnets" unsayable in a declarative config.
    ///
    /// One divergence from Go, because this fork splits Go's single field in two: the exit-node
    /// intent lives in [`advertise_exit_node`](ConfigVAlpha::advertise_exit_node) here, so an
    /// explicit `[]` does NOT stop this node advertising itself as an exit node — in Go the two
    /// default routes ride inside `AdvertiseRoutes`, so there `[]` with no `AdvertiseExitNode` key
    /// clears them too. Write `"AdvertiseExitNode": false` alongside the `[]` to stop both.
...
impl Config {
    ...
    /// advertised subnet route (Go's `if c.AdvertiseRoutes != nil` guard — see the field's docs,
    /// including why the exit-node advertisement survives it here). Each engine-gated / non-goal
    /// field that is *set to a non-default value* is logged at `warn` so a headless operator can see
    /// it was parsed but not applied (honest omission — never a silent drop).
    ///
    ...
    pub fn apply_to_prefs(&self, prefs: &mut Prefs) -> Result<Option<SecretString>> {
        ...
        // paths ask (`crate::routes::calc_advertise_routes`, Go `netutil.CalcAdvertiseRoutes` —
        // reached here through its `validate_advertise_routes` wrapper, which renders the same rules
        // as one message): every CIDR must parse at all (Go gets that for free — its
        // `AdvertiseRoutes` is a `[]netip.Prefix`, so a malformed entry dies in the JSON decode) and
        // be MASKED (`"192.0.2.5/24"` parses happily as an `IpNet` that keeps its host bits, so
        // without this the typo boots and the node advertises the route exactly as written instead
        // of the `192.0.2.0/24` the operator meant); every 4via6 prefix must actually decode; and a
        // default route must be advertised in BOTH families or neither, because a lone `0.0.0.0/0`
        // is a half exit node whose clients leak their IPv6 traffic out of their own link. That last
        // rule is asked of the routes this config COMPOSES with `AdvertiseExitNode` — the two are
        // one field in Go and two prefs here — and of the set that will actually be persisted, so a
        // config that does not mention routes at all is judged on the prefs it leaves in place,
        // while one that names an explicit `[]` is judged on the empty set it is about to write.
        //
        ...
        // Same rules and the same per-route wording as `Backend::check_prefs`, so the declarative and
        // interactive paths refuse the same configs; only the one-time `error parsing config to
        // prefs:` wrapper below is added here, matching Go, where `initPrefsFromConfig` /
        // `setConfigLocked` wrap `ToPrefs` and the CLI path does not. Every offender is collected
        // (Go `errors.Join`) rather than only the first: a headless deploy should learn about all its
        ...
        let prospective_routes = match &c.advertise_routes {
            ...
            // list to judge — an empty one withdraws every subnet route, which is a legal outcome.
        ...
        // Wrapped ONCE, in Go's words. Go's `ToPrefs` returns `errors.Join` of the bare route errors
        // and its two callers (`initPrefsFromConfig`, `setConfigLocked`) put `error parsing config to
        // prefs: %w` around the whole join. Prefixing every line with "config: " instead said the
        // same thing once per bad route and in nobody's wording; the wrapper goes on the front of the
        // joined block, so the lines below it are Go's verbatim.
        ...
        // `"AdvertiseRoutes": []` is present, and withdraws every subnet route this node advertised.
        // (The exit-node advertisement is a separate pref here and survives an explicit `[]`; see the
        // field's docs and the `AdvertiseExitNode` block below.)
...
mod tests {
    ...
    fn explicit_empty_advertise_routes_withdraws_them() {
        ...
        // withdrawing every subnet route this node advertised. Reading the field as a bare `Vec` made
        // an explicit `[]` indistinguishable from an absent key, so a declaratively managed subnet
        ...
        assert!(
            ...
            "an explicit [] withdraws every advertised subnet route, got {:?}",
            p.advertise_routes
        );

        // The one place this fork cannot reproduce Go's outcome: Go keeps the exit-node
        // advertisement INSIDE `AdvertiseRoutes`, so there a bare `[]` clears that too. Here the
        // intent lives in its own pref, and `[]` does not touch it — the node keeps advertising
        // both default routes, which also keeps the pairing rule satisfied. Stopping both takes
        // `"AdvertiseExitNode": false` in the same config.
    ...
    fn apply_refuses_a_malformed_4via6_route() {
        ...
        assert_eq!(
            e,
            "error parsing config to prefs: fd7a:115c:a1e0:b1a::/64 4-in-6 prefix must be at \
             least a /96"
        );
