...
/// string so a caller can tell the kinds apart (`Backend::check_prefs` folds them into its own error
/// list); [`Display`](std::fmt::Display) renders Go's wording, which is what every current caller
/// wants.
