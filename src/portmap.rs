...
///
/// Not an example address and not substitutable: `239.255.255.250` is the group the UPnP Device
/// Architecture assigns to SSDP, so it is part of the protocol exactly as port 1900 is. It is
/// administratively-scoped multicast (`239.0.0.0/8`) — link-local by definition, never routed off
/// the LAN — and it appears verbatim in the mandatory `HOST:` header of both M-SEARCH packets below.
pub const SSDP_MULTICAST: Ipv4Addr = Ipv4Addr::new(239, 255, 255, 250);

/// The generic SSDP M-SEARCH probe (Go `uPnPPacket`) — "tell me about everything you offer".
pub const UPNP_PACKET: &[u8] = b"M-SEARCH * HTTP/1.1\r\n\
HOST: 239.255.255.250:1900\r\n\
ST: ssdp:all\r\n\
MAN: \"ssdp:discover\"\r\n\
MX: 2\r\n\r\n";

/// The IGD-specific M-SEARCH probe (Go `uPnPIGDPacket`). Sent as well as the generic one because
/// some devices answer `ssdp:all` with only their *first* descriptor, which is often not the
/// gateway service we care about.
pub const UPNP_IGD_PACKET: &[u8] = b"M-SEARCH * HTTP/1.1\r\n\
HOST: 239.255.255.250:1900\r\n\
...
impl Client {
    ...
    pub async fn probe(&self) -> Result<ProbeResult, PortmapError> {
        ...
        if saw_upnp {
            ...
            let _ = sock.send_to(UPNP_PACKET, upnp_addr).await;
            let _ = sock.send_to(UPNP_PACKET, upnp_multicast_addr).await;
            let _ = sock.send_to(UPNP_IGD_PACKET, upnp_multicast_addr).await;
        }
                ...
                Some(PMP_VERSION) => {
                    ...
                    if pres.result_code != PmpResultCode::OK {
                        return Err(PortmapError::NoMapping(format!(
                            "no NAT mapping available: PMP response Op=0x{:x},Res=0x{:x}",
...
";
    ...
    async fn create_or_get_mapping_reports_a_nat_pmp_refusal_with_its_result_code() {
        ...
        assert_eq!(
            err,
            PortmapError::NoMapping(
                "no NAT mapping available: PMP response Op=0x80,Res=0x2".to_string()
            )
        );
