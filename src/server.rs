...
/// FIRST frame of a watch (the per-epoch front-load runs before the bus subscribe), so it is also a
/// frame the pending session id has to be able to ride out on. Returns `Err` if the client hung up (so
/// the caller returns `Ok(())` and ends the stream). Shared by the per-epoch front-load + both
/// suggestion select arms.
...
mod tests {
    ...
    /// frame that can legitimately be a watch's FIRST — the per-epoch front-load runs before the bus
    /// subscribe, so on a node that is already up it beats the engine's initial notify out the door.
    /// So it has to carry `version` and be able to carry the pending session id, the way Go's
    /// `sendToLocked` stamps whatever field triggered the send. Drives the production emitter over a
    /// `UnixStream` pair (no bind, no connect) and reads the line back off the wire.
    #[tokio::test]
