...
/// The `initial_status` snapshot is not affected either, for a different reason: it is one inline
/// write before the bus is ever subscribed, not a feed with a queue that can drop.
///
