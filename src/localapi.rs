...
pub enum Request {
    ...
    /// field (`suggested_exit_node` was the fifth, and `NotifyInitialSuggestedExitNode` is in neither
    /// refusal's operand set).
    ///
    ...
    Watch {
        ...
        /// ## What it costs
        ///
        /// Setting this bit buys ONE engine computation per watcher per device epoch: the front-load
        /// calls `Backend::suggest_exit_node`, which is an unbounded engine round-trip. Bounded and
        /// cheap for the watchers that exist today, but it is a real call, so a future caller added
        /// to that function is adding it to every such watch as well. The resulting bus traffic is
        /// already suppressed when the answer is stable (the publish is change-gated), so the cost is
        /// the computation, not the frames.
        ///
...
///   `NotifyInitialNetMap` and `suggested_exit_node` is `NotifyInitialSuggestedExitNode` (`1 << 10`),
///   none of which is in that set; `prefs` and `policy` are daemon-built and have no bit.
///
...
/// refusals, and Go's message goes in next to the check, verbatim. That is how
/// `suggested_exit_node` arrived: it is `NotifyInitialSuggestedExitNode`, which Go leaves out of
/// `NotifyRateLimitIncompatibleBits` and out of the in-process ruling, so it carries no refusal and
/// is named below with the rest. The return type is owned because one of the two messages
/// interpolates the offending bits.
...
mod tests {
    ...
    fn watch_usage_refusal_accepts_every_subscription_this_fork_can_spell() {
        ...
        // refuse if any rule applied to it. The loop bound is `1 << (number of mask fields)`, so a
        // new field is only really covered here once its author widens it — which is the same prompt
        // the named destructuring in `watch_usage_refusal` gives them.
        for bits in 0u8..32 {
            ...
            Response::Notify(back) => {
                ...
                assert!(
                    back.state.is_none()
                        && back.prefs.is_none()
                        && back.policy.is_none()
                        && back.suggested_exit_node.is_none()
                );
            }
            other => panic!("expected a notify frame, got {other:?}"),
        }
