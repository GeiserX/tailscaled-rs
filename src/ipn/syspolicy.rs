...
//! `preferencePolicies`) and the reload/change-callback half is
//! `util/syspolicy/rsop/resultant_policy.go` (`Reload`, `reloadNow`), both @
...
/// lands on the same items notifies nobody. That guard is ported here as [`reload_and_publish`],
/// which is the only thing that calls [`notify_policy_changed`].
///
...
/// the merge it produced differs from the one watchers already hold. Since the one source this
/// daemon registers is a JSON file captured at startup and never re-read, that means in practice
/// **registration is the only tick that can fire**: a `syspolicy reload` re-resolves the same rows
/// by construction, so it is silent. That is a coarser signal than Go's — a hand edit of
/// `syspolicy.json` is not seen until the daemon restarts — and the mask bit's documentation says
/// so rather than promising a watch this build cannot perform. A file watcher would narrow the gap
...
/// ```
///
/// Returns the fresh merge, so the caller reports what it published.
///
...
pub fn load_json_policy_file(source_name: &str, path: &Path) -> Result<LoadOutcome, String> {
    ...
    // an older snapshot (it does whenever the file configured at least one key). Nothing is watching
    // at daemon start (this runs from `main` before the LocalAPI is served), so this is for the sake
    // of the invariant rather than any current caller — every mutation of `REGISTERED` is published,
    // so a watcher can never silently miss one.
...
/// effective policy CHANGED, and a frame that carries the rows a watcher already has says the
/// opposite of what the watcher would conclude from receiving it.
...
mod tests {
    ...
    fn only_a_resolve_that_moves_the_snapshot_pushes_a_policy_frame() {
        ...
        // A value that moved is, even though the key set did not — `EqualItems` compares the rows.
