...
/// [`retriggers`](captive::ConnectivityWarnable::retriggers) — and a steady warnable only when no
...
/// every comparison and then — having probed its one time — hold the episode silent for as long as
/// the operator leaves the sysctl off, even if the host network died an hour later. Upstream has no
/// such trap: a second warnable going unhealthy is itself a health change, which re-arms its timer.
/// Preferring the warnable whose condition keeps producing changes is how that survives here.
...
impl Backend {
    ...
    ///
    /// Both of upstream's gates are reproduced. `applyPrefsToHostinfoLocked`
    /// (`ipn/ipnlocal/local.go`) installs the check only when the node advertises at least one route
    /// — `len(hi.RoutableIPs) > 0`, where an advertised exit node *is* two routes, `0.0.0.0/0` and
    /// `::/0` — and only when the node is not a netstack router; in every other case it calls
    /// `SetIPForwardingCheck(nil)`, which sets the warnable **healthy**. The netstack gate lives
    /// inside [`forwarding_broken`](crate::ipforward::forwarding_broken), where it is the same
    /// `tun_enabled` short-circuit the `check-ip-forwarding` LocalAPI already uses; the route gate is
    /// here, because only the backend knows the prefs.
    ///
...
mod tests {
    ...
    #[tokio::test]
    async fn ip_forwarding_health_honours_gos_two_gates() {
        ...
        // An advertised exit node IS routable IPs to Go (0.0.0.0/0 + ::/0), so it opens the gate on
        // its own, with no explicit --advertise-routes.
        ...
        // Routes + kernel TUN: the check runs. Off Linux, Go's `netutil.CheckIPForwarding` is a
        // no-op, so the answer is healthy; on Linux it is whatever the host's sysctls say, and the
        // only invariant a unit test can hold is that an unhealthy verdict has a warning behind it.
        be.prefs.tun_enabled = true;
    ...
    fn each_connectivity_warnable_carries_gos_settle_time() {
        ...
        assert_eq!(
            ...
            "ip-forwarding-off sets no TimeToVisible, and `Warnable.IsVisible` treats zero as \
             visible at once, so only Go's 2s detection interval is owed"
        );

        // The other half of the per-warnable behaviour: which of them upstream would keep
        // re-triggering on while the condition holds.
        assert!(NetworkStatus.retriggers() && NoDerpHome.retriggers());
        assert!(
            !IpForwardingOff.retriggers(),
            "a sysctl and a pref produce no further health changes, so Go probes them once"
        );
    }
