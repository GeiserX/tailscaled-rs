...
/// here where Go yields that runner-up. That outcome is NOT Go's empty response — it only coincides
/// with it when the permitted set happens to be empty — so it does not get to look like one:
