...
/// and they are the same rows by construction: both come from `Backend::policy_snapshot`. That file
/// is also where the *positive* edge is pinned end to end — registering a source takes the effective
/// policy from empty to its rows and does wake a watcher — because registration is process-global
/// and this binary's other fourteen tests must keep resolving an empty policy.
///
