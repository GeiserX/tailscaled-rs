...
#[test]
fn host_routes_refusal_is_gos_line_byte_for_byte_and_nothing_more() {
    // What "carries Go's refusal" means, pinned so the README and the doc comments can say it
    // without a reader having to take their word for it: the fork prints Go's sentence exactly,
    // and prints nothing else.
    //
    // Three ways that claim can rot. Drift in the sentence — a reworded "only 'true' is allowed",
    // an `Error: ` prefix back in front — makes the README's "Go's own" false in the direction that
    // matters, because an operator grepping Go's text would stop matching. The flag name is the
    // subtle one: Go's flag package prints the name as registered, `-host-routes` with ONE dash,
    // even when the operator typed `--host-routes`, so echoing the typed `--` spelling back is a
    // divergence too. And Go's `failf` prints the message and then calls `f.usage()` (Go
    // `src/flag/flag.go`), so upstream's stderr is the sentence FOLLOWED BY the command's whole flag
    // list; this fork stops at the sentence. Equality against the one expected line pins all three.
    //
    // `up` and `login` share Go's one flag set, so both must print the same line.
    for cmd in ["up", "login"] {
        let out = tnet(&[cmd, "--host-routes=false"]);
        let stderr = String::from_utf8_lossy(&out.stderr);
        ...
        assert_eq!(
            printed,
            vec![GO_HOST_ROUTES_LINE],
            "`tnet {cmd} --host-routes=false` should print Go's refusal byte for byte — one-dash \
             `-host-routes` included — and no usage block after it; got: {stderr}"
        );
    }
}
...
fn nickname_is_distinguished_from_the_flags_that_are_only_renames() {
    ...
    // `host_routes_refusal_is_gos_line_byte_for_byte_and_nothing_more`). Exiting 1 here would tell a
