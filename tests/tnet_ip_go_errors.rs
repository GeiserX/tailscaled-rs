...
impl RealDaemon {
    /// Start the daemon over a fresh state dir. The two seeds pick the backend state the message
    /// has to name, via `derive_state_from(has_device=false, …)` (src/ipn/state.rs):
    ///
    /// * `prefs` is written to `prefs.json` before the load. Absent ⇒ a never-configured node;
    ///   `want_running` with no engine ⇒ `NeedsLogin`.
    /// * `node_key` writes a freshly generated `node.key.json` in the daemon's own on-disk shape,
    ///   which is what makes a node `Stopped` rather than `NoState` — Go gates `Stopped` on a
    ///   persisted node key too (`hasNodeKeyLocked`), and `Stopped` is the state an operator who
    ///   ran `tnet down` actually sees.
    fn start(prefs: Option<&str>, node_key: bool) -> RealDaemon {
        let n = UNIQUE.fetch_add(1, Ordering::Relaxed);
        ...
        if node_key {
            // The daemon reads this file as `{ "key_state": <PersistState> }` and only asks whether
            // it parses (`key_file_has_node_key`), so a freshly generated state (the engine's
            // `Default`, which randomises every key) is the cheapest honest "this node has
            // registered before" — no control server, no network.
            let key_state = tailscale::keys::PersistState::default();
            let file = serde_json::json!({ "key_state": key_state });
            std::fs::write(
                state_dir.join("node.key.json"),
                serde_json::to_vec(&file).expect("serialize the seeded key state"),
            )
            .expect("seed node.key.json");
        }
...
fn the_real_addressless_daemon_reports_gos_state_line() {
    for (prefs, node_key, state) in [
        // Never configured: no prefs file, no key.
        (None, false, "NoState"),
        // Registered once, then brought down — the `tnet down` node, and the state the original
        // report named.
        (None, true, "Stopped"),
        // Wants to be up but has no engine — Go's `NeedsLogin`.
        (Some(r#"{"want_running":true}"#), false, "NeedsLogin"),
    ] {
        let daemon = RealDaemon::start(prefs, node_key);
        for args in [&["ip"][..], &["ip", "-4"], &["ip", "-6"], &["ip", "-1"]] {
            let out = daemon.tnet(args);
            assert_eq!(
                out.status.code(),
...
fn the_real_addressless_daemon_fails_an_assertion_with_an_empty_list() {
    let daemon = RealDaemon::start(None, false);
...
/// which every other one of Go's own refusals in this command had already stopped doing.
#[test]
